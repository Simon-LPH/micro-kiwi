 ...
 * @author chenzhouyu
 * 类用法
 * $checkcode = new checkcode("../../font/elephant.ttf");
 * $checkcode->doimage();
 * //取得验证
...
class Checkcode {
	...
	private function creat_font() {
		...
		for ($i=0; $i<$this->code_len; $i++) {
			imagettftext($this->img, $this->font_size, rand(-30,30), $x*$i+rand(0,3), $this->height/1.4, $this->font_color, $this->font, $this->code[$i]);
